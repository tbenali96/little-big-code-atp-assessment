...
def plot_rankings_for_player_per_year(data: pd.DataFrame, player: str) -> plotly.graph_objs._figure.Figure:
    new_dataframe = data[data.p1_name == player]\
        .sort_values(by=["date"], ascending=False)\
        .drop_duplicates(["year"], keep="first")
    fig = px.bar(new_dataframe, x="year", y="p1_rank", title=f"Last ranking of {player} per year")
    fig.update_yaxes(autorange="reversed")
    return fig


def plot_number_of_games_per_year(data: pd.DataFrame, player: str)  -> plotly.graph_objs._figure.Figure:
    new_dataframe = data[data.p1_name == player]
    fig = px.histogram(new_dataframe, x="year", color="p1_won", title=f"Number of games played by {player}")
    return fig


def extract_date(df: pd.DataFrame) -> pd.DataFrame:
    df["date"] = df["tourney_date"].apply(lambda x: pd.to_datetime(str(x), format='%Y%m%d'))
